...
| External asset layer | 25 packed CC0 image assets, 43 normalized runtime textures, 24 derived UI item icons |
...
- `test:assets` checks that normalized cache sprites, cache-opening frames, sliced sheet icons, derived CSS icons, and recover-heart FX are actually instantiated, not only present in the packed asset bundle.
